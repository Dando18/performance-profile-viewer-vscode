...
function openProfile(schemes: string[]): void {
  ...
  vscode.window
    ...
    .then((profileType: string | undefined) => {
      if (profileType) {
        const isDirectory =
          PROFILER_OUTPUT_TYPES[
            profileType as keyof typeof PROFILER_OUTPUT_TYPES
          ].isDirectory;
...
export function activate(context: vscode.ExtensionContext) {
  ...
  let openProfileCommand = vscode.commands.registerCommand(
    "profileviewer.openProfile",
  ...
  let openProfileTreeCommand = vscode.commands.registerCommand(
    "profileviewer.openProfileTree",
  ...
  let openProfileFlameGraphCommand = vscode.commands.registerCommand(
    "profileviewer.openFlameGraph",
  ...
  const pyinstrumentTaskProvider = new ProfilerTaskProvider(
    "PyInstrument",
    "PyInstrument",
  );
  vscode.tasks.registerTaskProvider(
    pyinstrumentTaskProvider.taskType,
    pyinstrumentTaskProvider,
  );

  const cProfileTaskProvider = new ProfilerTaskProvider("cProfile", "cProfile");
  vscode.tasks.registerTaskProvider(
    cProfileTaskProvider.taskType,
    cProfileTaskProvider,
  );

  const hpctoolkitTaskProvider = new ProfilerTaskProvider(
    "HPCToolkit",
    "HPCToolkit",
  );
  vscode.tasks.registerTaskProvider(
    hpctoolkitTaskProvider.taskType,
    hpctoolkitTaskProvider,
  );
}
