...
export class ProfilerOutputNode {
  ...
  public static fromObject(obj: any): ProfilerOutputNode {
    ...
    const children = obj.children.map((node: any) =>
      ProfilerOutputNode.fromObject(node),
    );
    ...
    if (recursive) {
      for (const child of this.children) {
        maxMetricValue = Math.max(
          maxMetricValue,
          child.getMaxMetricValue(metricName, recursive),
        );
      }
    }
  ...
  public isOnHotPath(): boolean {
    return (
      this.attributes.hasOwnProperty("hot_path") && this.attributes["hot_path"]
    );
  }
      ...
      if (workspaceRoot) {
        if (
          await this.fileExists(
            vscode.Uri.joinPath(workspaceRoot, filename).fsPath,
          )
        ) {
...
export class ProfilerOutputTree {
  ...
  public getMaxMetricValue(metric: string, recursive: boolean = true): number {
    ...
    for (const root of this.roots) {
      maxMetricValue = Math.max(
        maxMetricValue,
        root.getMaxMetricValue(metric, recursive),
      );
    }
    return maxMetricValue;
  }
  ...
  public getMaxInclusiveTime(): number {
    ...
    return Math.max(
      ...this.roots.map(
        (root: ProfilerOutputNode) => root.getInclusiveTime() || 0,
      ),
    );
  }

  public setValueMetric(metricColumn: string, recursive: boolean = true) {
    this.roots.forEach((root: ProfilerOutputNode) =>
      root.setValueMetric(metricColumn, recursive),
    );
  }
  ...
  public getAvailableMetrics(
    ...
    if (removePrefix && removePrefix.length > 0) {
      availableMetrics = availableMetrics.filter(
        (metric: string) => !metric.startsWith(removePrefix),
      );
    }
...
export class ProfilerOutput implements vscode.Disposable {
  ...
  private context:
    ...
    | undefined; /* optionally get info from context */
  ...
  public static fromUri(uri: vscode.Uri): ProfilerOutput {
    ...
    const profileInfo =
      PROFILER_OUTPUT_TYPES[query.type as keyof typeof PROFILER_OUTPUT_TYPES];
  ...
  public getTree(): Thenable<ProfilerOutputTree> {
    ...
    return new Promise<ProfilerOutputTree>((resolve, reject) => {
      ...
      pythonPathPromise.then((pythonPath) => {
        ...
        const pythonScriptUri = vscode.Uri.joinPath(
          extensionUri,
          "src",
          "parse_profile.py",
        );
