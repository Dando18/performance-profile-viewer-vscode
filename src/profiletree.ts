...
export class ProfileTreeEditor implements vscode.CustomReadonlyEditorProvider {
  ...
  private readonly htmlPath: string = path.join(
    "src",
    "html",
    "profiletree.html",
  );
  ...
  constructor(context: vscode.ExtensionContext, initialLevelsOpen: number = 3) {
    ...
    const htmlUri: vscode.Uri = vscode.Uri.joinPath(
  ...
  private getHotPathIcon(node: ProfilerOutputNode): string {
    ...
    if (
      vscode.workspace
        .getConfiguration("profileviewer")
        .get("animatedHotPathIcons") === true
    ) {
  ...
  private getMetricsAsHTMLOptions(
  ...
  private async getHtmlNestedLists(
    ...
    const metricElem = this.getMetricHtmlElement(
      node.getMetricValue(metric) || 0,
      maxMetricValue,
    );
    const filenameElem = this.getFilePathHtmlElement(
      node.getFilename() || "",
      resolvedFilename,
      line,
    );
    ...
    if (node.children && node.children.length > 0) {
      ...
      node.children.sort((a, b) => {
        return (
          (b.getMetricValue(metric) || 0) - (a.getMetricValue(metric) || 0)
        );
      });
      ...
      for (let child of node.children) {
        if (child) {
          htmlContent += await this.getHtmlNestedLists(
            child,
            metric,
            maxMetricValue,
            depth + 1,
  ...
  private async getHtmlForWebview(
    ...
    for (let root of tree.roots) {
      ...
      htmlLists += await this.getHtmlNestedLists(
        root,
        metric,
        maxMetricValue,
        0,
    ...
    htmlContent = htmlContent.replace(
      "{{ availableMetrics }}",
      this.getMetricsAsHTMLOptions(tree, metric),
    );
    htmlContent = htmlContent.replace(
      "{{ codiconsUri }}",
