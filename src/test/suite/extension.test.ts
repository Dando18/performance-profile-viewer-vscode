...
suite("Environment Tests", () => {
  ...
  test("External Python is cached", async () => {
    const pythonPath = await findPythonWithCache(
      extensionContext,
      ["hatchet", "numpy"],
      true,
    );
    ...
    assert.ok(
      extensionContext.workspaceState.get<string>("pythonWithHatchetPath"),
    );
    assert.strictEqual(
      extensionContext.workspaceState.get<string>("pythonWithHatchetPath"),
      pythonPath,
...
suite("Profile Parsing Tests", () => {
  ...
  test("Open ScoreP Profile", async () => {
    ...
    if (hatchetVersion === "1.3.1") {
      assert.ok(
        Math.abs(tree.getMaxMetricValue("max_time (inc)") - 5.0556) < 0.0001,
      );
    } else {
