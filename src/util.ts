...
function doesPythonHaveModules(pythonPath: string, modules: string[]): boolean {
  try {
    let output = execSync(
      `${pythonPath} -c \"import ${modules.join("; import ")};\"`,
    );
    if (output) {
      return true;
    }
...
export async function findPythonWithCache(
  context: vscode.ExtensionContext,
  imports?: string[],
  useCached: boolean = true,
): Promise<string | undefined> {
  if (useCached) {
    const cachedPython = context.workspaceState.get<string>(
...
export async function getHatchetVersion(
  pythonPath?: string | vscode.Uri,
): Promise<string> {
  pythonPath = pythonPath || (await getPythonPath());
  let output = execSync(
    `${pythonPath} -c "import hatchet; print(hatchet.version.__version__)"`,
  );
